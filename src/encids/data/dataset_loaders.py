...
_ISCX_CATEGORIES = {
    ...
    "vimeo": "streaming", "spotify": "streaming", "hangouts_audio": "voip",
    "skype_audio": "voip", "voipbuster": "voip", "facebook_audio": "voip",
    "torrent": "p2p", "bittorrent": "p2p",
}


def _iscx_label(path: Path) -> str:
    """Derive a traffic-category label from an ISCX pcap filename."""
    name = path.stem.lower()
    tunnelled = name.startswith("vpn") or "_vpn" in name
    category = next((cat for key, cat in _ISCX_CATEGORIES.items() if key in name),
                    "web_browsing")
...
def load_iscx_vpn2016(root: str | Path, backend: str = "auto",
    ...
    df = pcaps_to_dataframe(pcaps, label_fn=_iscx_label, backend=backend)
    if df.empty:
        return df
...
_DARKNET_MALICIOUS = {"tor", "vpn", "darknet"}   # "darknet" per Label column
